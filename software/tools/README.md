...
Scripts utilitarios para barridos, validación y operación asistida.
...
- `bench_turbo_validation.py`: rutina de banco en modo PID para medir tiempo de
  subida y overshoot con distintas máscaras de cámara; usa comando atómico en
  `AUTO`.
- `pump_swap_validation.py`: benchmark para comparar configuraciones de bombas
  ...
  usa comando atómico en `AUTO`.
- `pump_eval_core.py`: núcleo de evaluación dual para selección de bomba (capacidad
  y tiempos en presión/vacío), con score balanceado y registro histórico. Las
  fases PWM usan `DIRECT`; las fases por target usan `AUTO`.
...
- `hardware_runtime_check.py`: verifica el grafo ROS activo y el contrato mínimo
  del firmware, incluyendo `/pneumatic_state`.
